...
- **Validate before running**: `overwire validate --config-root <app>/.overwire --json` from this directory (replace `<app>` with hello-app, pipeline-app, compliance-app, or enterprise-actions). compliance-app intentionally reports findings from `bad-practices.yml` — it exists as a lint target.
